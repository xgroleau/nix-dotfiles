# Code from
# https://github.com/davidtwco/veritas/blob/2355ea0dee15252b3327c2e1a28106cfb10cf766/nixos/modules/per-user-vpn.nix
{ config, lib, pkgs, ... }:

# This module defines the `modules.networking.forced-vpn` options for setting up an OpenVPN
...
with lib.options;
with lib.types;
let cfg = config.modules.networking.forced-vpn;
in {
  options.modules.networking.forced-vpn = {
    ...
    servers = mkOption {
      ...
      type = attrsOf (submodule {
        options = {
          ...
          dns = {
            ipv4 = {
              ...
              secondary = mkOption {
                default = "1.0.0.1";
                description =
                  "Secondary IPv4 DNS server to use for VPN traffic.";
                type = str;
              };
            };
            ...
            ipv6 = {
              ...
              secondary = mkOption {
                ...
                description =
                  "Secondary IPv6 DNS server to use for VPN traffic.";
                type = str;
              };
            };
          };
          ...
          users = mkOption {
            description =
              "Names of users whose traffic will be forced onto the VPN.";
            type = listOf str;
          };
        };
      });
        ...
        forceDnsForUser = user:
          let
            ...
            redirectDnsPacketForUser =
              { command, dns, networks, protocol, user }: ''
    ...
    mkRoutingScript = srvName: srv:
      let
        ...
        dir = pkgs.writeScriptBin name ''
          ...
          if [[ $HAS_TABLE == "1" ]]; then
            ${pkgs.iproute}/bin/ip route flush table ${
              builtins.toString srv.routeTableId
            }
          fi
  ...
  in {
    ...
    } // mapAttrs' (name: _: nameValuePair "net.ipv4.conf.${name}.rp_filter" 2)
      cfg.servers;
    ...
    # requests.
    networking.nameservers = flatten (mapAttrsToList (_: srv:
      with srv.dns; [
        ipv4.primary
        ipv4.secondary
        ipv6.primary
    ...
    systemd.services = mapAttrs' (name: srv:
